...
def magic_number(a_num):
    """ Return boolean True if the parameter a_num contains the magic number 7. If not answer boolean False """
    magic_number = "7"
    if magic_number in str(a_num):
        return print("True")
    else:
        return print("False")
...
def hms_to_s(hours, minutes, seconds):
    """ Convert a duration of hours:minutes:seconds to seconds """
    the_result = seconds1 = (hours*3600) + (minutes*60) + seconds
    return the_result
...
def add_hms(hr1, min1, sec1, hr2, min2, sec2):
    ...
    minutes2 = ((s%3600) // 60)
    seconds2 = ((s%3600) % 60)
    the_result = hours2, minutes2, seconds2
    return the_result    
...
def add_old_uk(pounds1, shillings1, pennies1, pounds2, shillings2, pennies2):
    ...
    the_result = pounds, shillings, pennies
    return the_result
...
def lucky_fun(an_int):
    def additional_fun(inner_num):
        lucky_fun = str(an_int)
        if lucky_fun in str(inner_num):
            return print ("True")
        else:
            return print ("False")
        return str(an_int) in str(inner_num)
    return (additional_fun)
...
def rearranged(phrase1, phrase2):
    """ Return True if parameters contain exactly the same letters, possibly arranged differently. False otherwise """
...
def is_back_to_front(phrase):
    """ Return parameter True if string reads the same backwards as forwards, else False """
    set2 = set(str(phrase))
