...
def all_ops_pt1(int1, int2):
    ...
    return int1 + int2, int1 - int2, int1 * int2, int1 / int2, int1 // int2, int1 % int2 # your, return, values
...
def magic_number(a_num):
    ...
    return "7" in str(a_num) # either boolean True or Boolean False
...
def hms_to_s(hours, minutes, seconds):
    ...
    a = (hours * 3600) # hours to seconds
    b = (minutes * 60) # minutes to seconds
    return a + b + seconds # the integer number of seconds
...
def s_to_hms(seconds):
    ...
    m = (seconds-(h * 3600)) // 60
    s = (seconds-(h * 3600)) % 60
    return h, m, s # the tuple (hours, minutes, seconds)
...
def add_hms(hr1, min1, sec1, hr2, min2, sec2):
    ...
    time_raw = hms_to_s(hr1, min1, sec1) + hms_to_s(hr2, min2, sec2) # raw value in seconds
    return s_to_hms(time_raw) # the tuple (hours, minutes, seconds)
...
def add_old_uk(pounds1, shillings1, pennies1, pounds2, shillings2, pennies2):
    ...
    def psp_to_pen(pounds, shillings, pennies):
        a = (pounds * 240) # pounds to pennies
        b = (shillings * 12) # shillings to pennies
        return a + b + pennies

    def pen_to_psp(pennies):
        pou = pennies // 240
        s = (pennies-(pou * 240)) // 12
        pen = (pennies-(pou * 240)) % 12
        return pou, s, pen

    a = psp_to_pen(pounds1, shillings1, pennies1) + psp_to_pen(pounds2, shillings2, pennies2)

    return pen_to_psp(a) # the tuple (pounds, shillings, pence)
...
def lucky_fun(an_int):
    ...
    def lucky_number(inner_num):
        return str(an_int) in str(inner_num).replace('.', '') # either boolean True or Boolean False
    return lucky_number # a function
...
def rearranged(phrase1, phrase2):
    ...
    a = "".join(phrase1.lower()).replace(" ","")
    b = "".join(phrase2.lower()).replace(" ","")
    
    return sorted(a) == sorted(b) # either boolean True or Boolean False
...
def is_back_to_front(phrase):
    ...
    a = "".join(phrase.lower()).replace(" ","")
    return a == a[::-1] # either boolean True or boolean False
