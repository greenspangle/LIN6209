...
def magic_number(a_num):
    ...
    for digit in str(a_num):
        if digit != "-" and digit != "." and int(digit) == 7:
            magic = True
            break
        else:
            magic = False
...
def s_to_hms(seconds):
    ...
    mins = ((seconds - remseconds) % 3600) / 60
    hours = (seconds - remseconds - 60 * mins) / 3600
    hms = str(int(hours)) + ":" + str(int(mins)) + ":" + str(int(remseconds))
    return hms
...
def add_old_uk(pounds1, shillings1, pennies1, pounds2, shillings2, pennies2):
    ...
    pounds = 0
    shillings = 0
    allpennies = pennies1 + pennies2
    if allpennies >= 12:
        while allpennies >= 12:      
            allpennies -= 12
            shillings += 1
        rempennies = allpennies
    else:
        rempennies = allpennies
    ...
    if shillings >= 20:
        while shillings >= 20:      
            shillings -= 20
            pounds += 1
        remshillings = shillings
    else:
        remshillings = shillings
    pounds += pounds1 + pounds2
    return pounds, remshillings, rempennies
...
def lucky_fun(an_int):
    ...
    numofdigits = 0
    for digit in str(an_int):
        numofdigits += 1
    def find_num(numbertosearch):
            isin = False
            numbertosearch = subpunct(str(numbertosearch))
            numberlen = len(numbertosearch)
            targetlen = len(str(an_int))
            if numberlen < targetlen:
                isin = False
            else:
                counter = targetlen
                chopmax = numberlen
                while counter <= chopmax:
                    truncated = (numbertosearch[counter - targetlen:counter])
                    if str(truncated) == str(an_int):
                        isin = True
                        break
                    else:
                        counter += 1
...
def rearranged(phrase1, phrase2):
    ...
    nospace1 = phrase1.replace(" ", "")
    nospace2 = phrase2.replace(" ", "")
    letters1 = letters(nospace1.lower())
    letters2 = letters(nospace2.lower())
    letters1 = sorted(letters1)
    letters2 = sorted(letters2)
    return str(letters1) == str(letters2)

# Helper functions
def letters(stringtosearch):
    return {character for character in stringtosearch}
def subpunct(st):
	st = st.replace("-", "")
	st = st.replace(".", "")
	return st
...
def is_back_to_front(phrase):
    ...
    return nospace.lower() == nospace[::-1].lower()
